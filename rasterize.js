...
//const INPUT_TRIANGLES_URL = "https://raw.githubusercontent.com/Lamonkey/jasonTest/master/testSort7.json";
//const INPUT_TRIANGLES_URL = "https://ncsucgclass.github.io/prog3/triangles2.json";
var defaultEye = vec3.fromValues(0.5, 0.5, -0.5); // default eye position in world space
var defultCenter = vec3.fromValues(0.5, 0.5, 0.5); // default view direction in world space
var defultUp = vec3.fromValues(0, 1, 0); // default view up vector
...
var Center = vec3.clone(defultCenter); // view direction in world space
var Up = vec3.clone(defultUp); // view up vector in world space
var isPerspectiveProjection = true;
var litLoc = true; // switch between lgihtmap and unlit;
var useTextureLoc;
...
function handleKeyDown(event) {
    ...
    switch (event.code) {
        ...
        // model selection
        case "KeyU": //show unlit 
            gl.uniform1i(litLoc, false);
            Blinn_Phong = false;
            break;
        case "KeyM": //show light map
            gl.uniform1i(litLoc, true);
            Blinn_Phong = false;
            break;
        case "Space":
            if (handleKeyDown.modelOn != null)
        ...
        case "Escape": // reset view to default
            ...
            Center = vec3.copy(Center, defultCenter);
            Up = vec3.copy(Up, defultUp);
            break;
        ...
        if (inputTriangles == String.null)
            ...
            for (var whichSet = 0; whichSet < numTriangleSets; whichSet++) { // for each tri set
                ...
                for (whichSetVert = 0; whichSetVert < numVerts; whichSetVert++) { // verts in set
                    ...
                    inputTriangles[whichSet].glTexCoords.push(uvToAdd[0], uvToAdd[1]);
...
function setupShaders() {
    ...
    var fShaderCode = `
        ...
        uniform sampler2D lSampler; //sampler for light map
        uniform bool unlit; // switch between light map and unlit
        uniform float alpha; // alpha of triangle
        uniform bool useTexture;//used texture
        ...
        void main(void) {
            ...
            vec4 texture = texture2D(lSampler, vTextureCoord);
            vec4 lightMap = texture2D(uSampler, vTextureCoord);
            vec4 textureOut;
            
            
            
            if(unlit)  
                textureOut = texture * lightMap;
            else if(!unlit)
                textureOut = texture ;
           
                if(Blinn_Phong){
                    gl_FragColor = vec4(colorOut * textureOut.rgb, alpha * textureOut.a);
                }
                else{
                    // if(unlit)  
                    //     textureOut = lightMap;
                    // else if(!unlit)
                    //     textureOut = texture * lightMap;
                    gl_FragColor = vec4(textureOut.rgb, alpha * textureOut.a);
                }
    ...
    try {
        ...
        if (!gl.getShaderParameter(fShader, gl.COMPILE_STATUS)) { // bad frag shader compile
            ...
            if (!gl.getProgramParameter(shaderProgram, gl.LINK_STATUS)) { // bad program link
                ...
                samplerULoc = gl.getUniformLocation(shaderProgram, "uSampler"); // ptr to samplers
                samplerlLoc = gl.getUniformLocation(shaderProgram, "lSampler");
                litLoc = gl.getUniformLocation(shaderProgram, "unlit");
...
function renderModels() {
    ...
    for (var whichTriSet = 0; whichTriSet < numTriangleSets; whichTriSet++) {
        ...
        if (currSet.material.alpha == 1.0)
            opaqueTri.push(currSet);
        else
            transparentTri.push(currSet);
    }
    // render each triangle set
    function renderTri(triSet) {
        var currSet; // the tri set and its material properties
        for (var whichTriSet = 0; whichTriSet < triSet.length; whichTriSet++) {

            currSet = triSet[whichTriSet];
    ...
    for (var i = 0; i < inputTriangles.length; i++){
        if(inputTriangles[i].material.alpha == 1.0) //is opaque
            opaqueTri.push(inputTriangles[i]);
        else
            transTri.push(inputTriangles[i]);
    }
    //render opaque models
    gl.enable(gl.DEPTH_TEST);
    gl.depthMask(true);
    renderTri(opaqueTri);
    //rendet transparent set
    gl.enable(gl.BLEND);
...
function loadLightMap() {
    ...
    image.src = "https://ncsucgclass.github.io/prog4/lightmap.gif";
    return texture;
}
...
function main() {
   // wall = new wall([1,1,1])
    ...
    lightMap = loadLightMap();
      //sort by depth buffer
     inputTriangles.sort(function (a, b) { return a.center[2] - b.center[2] });
      inputTriangles.reverse();
